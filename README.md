...
the command line option has priority.
...
**NOTE:**
For optimal (even) connection distribution the number of connections must be equal to the product of number of read replicas times N, where N is positive integer.

C = R x N

Where C - number of concurrent connections, R - number of read DB instances, N - positive integer.
